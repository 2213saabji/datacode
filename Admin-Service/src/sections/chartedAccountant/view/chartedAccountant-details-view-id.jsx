...
import { useGetCharteredAccountant, useGetCharteredaccountants } from 'src/api/chartedAccountant';
...
export default function ChartedAccountantDetailsViewById() {
  const { user } = useAuthContext();
  const { charteredaccountants, charteredaccountantsLoading } = useGetCharteredaccountants();
 console.log("All Ca",charteredaccountants);
  const [chartedAccountantData, setchartedAccountantData] = useState({});
  const [myCa, setMyCa] = useState(false);
  const [openDialog, setOpenDialog] = useState(false);
 
  useEffect(() => {
    if (charteredaccountants?.data) {
      const matchedCa = charteredaccountants?.data?.filter(
        (service) => service?.User?.UserId === user?.userId
      );
      console.log('matchedLawyer', matchedCa);
      if (matchedCa.length > 0) {
        setMyCa(matchedCa[0]?.providerId);
      } else {
        setMyCa(6);
      }
    }
  }, [charteredaccountants?.data, user?.userId]);
 
  const id = myCa;
 
  const { chartedaccpunt, chartedaccpuntIdError } = useGetCharteredAccountant(id);
  console.log("chartedaccpunt",chartedaccpunt)
  useEffect(() => {
  ...
  const renderPost = chartedaccpunt && (
    <>
      ...
      >
        <CustomBreadcrumbs
          links={[{ name: 'Chartered Accountant' }, { name: 'Details' }]}
        ...
        <Stack sx={{ maxWidth: 720, mx: 'auto' }}>
          <Stack direction="column" alignItems="start">
            ...
            <Stack direction="row" alignItems="start" sx={{ marginTop: '10px' }}>
              ...
              <Typography sx={{ ml: 1 }}>{chartedaccpunt?.data?.providerName}</Typography>
            </Stack>
            <Stack direction="row" alignItems="start" sx={{ marginTop: '10px' }}>
              <Typography sx={{ mr: 1, minWidth: 180 }}>Experience Level:</Typography>
              <Typography sx={{ ml: 1 }}>{chartedaccpunt?.data?.experienceLevel}</Typography>
            </Stack>
            <Stack direction="row" alignItems="start" sx={{ marginTop: '10px' }}>
              <Typography sx={{ mr: 1, minWidth: 180 }}>Rating:</Typography>
              <Typography sx={{ ml: 1 }}>{renderStars(chartedAccountantData?.data?.rating)}</Typography>
            </Stack>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={12}>
              <Card sx={{ borderRadius: 2, boxShadow: 3 }}>
                <CardContent>
                  ...
                  <Typography>Name: {chartedaccpunt?.data?.providerName}</Typography>
                  <Typography>Experience Level: {chartedaccpunt?.data?.experienceLevel}</Typography>
            ...
            <Grid item xs={12} md={16}>
              <Card sx={{ borderRadius: 2, boxShadow: 3, mb: 3 }}>
                <CardContent>
                  <Typography variant="h6">Case Photos</Typography>
      ...
      <Dialog open={openDialog} onClose={handleDialogClose} fullWidth>
        ...
        <DialogContent>
          ...
          <Typography>{chartedAccountantData.providerName}</Typography>
          <Typography variant="h6" sx={{ mt: 2 }}>
            Experience Level:
          </Typography>
          <Typography>{chartedAccountantData.experienceLevel}</Typography>
          <Typography variant="h6" sx={{ mt: 2 }}>
            Rating:
          </Typography>
