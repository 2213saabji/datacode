...
import DriverAnalytic from '../sub-service-analytic';
// import ProductTableToolbar from '../driver-table-toolbar';
import ProductTableFiltersResult from '../sub-service-table-filters-result';
import {
  RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
  RenderCellCreatedAt,
} from '../sub-service-table-row';
...
export default function SubServiceListView() {
  ...
  const { subservices, lawyersLoading } = useGetSubServices();
  ...
  useEffect(() => {
    console.log('line-94', subservices);
    if (subservices && subservices.data) {
      // console.log("subservice", subservices.data);
      const updatedTableData = subservices.data.map((service) => ({
        ...service,
        id: service.issueId,
      }));
      // console.log('updated table', updatedTableData)
      setTableData(updatedTableData);
    }
  }, [subservices]);
  // console.log('tablre', tableData)
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `/legalIssue/delete/${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const apiUrl = `/legalIssue/delete`;
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.LMS_sub_service.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.LMS_sub_service.details(id));
    },
    [router]
  );
  ...
  const columns = [
    ...
    // {
    //   field: 'issueId',
    //   headerName: 'Sub Services Offered',
    //   flex: 1,
    //   minWidth: 360,
    //   hideable: false,
      ...
      >
        <CustomBreadcrumbs
          ...
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.LMS_sub_service.create}
              variant="contained"
