...
import {useGetLawyer,useGetLawyers} from 'src/api/lawyer';
...
export default function VendorDetailsViewById() {
  ...
  const [myLawyer, setMyLawyer] = useState(false);

  console.log("user",user)

  console.log("lawyers",lawyers?.data);
  useEffect(() => {
    if (lawyers?.data) {
      const matchedLawyer = lawyers?.data?.filter(service => service?.User?.UserId === user?.userId);
      console.log("matchedLawyer",matchedLawyer);
      if (matchedLawyer.length > 0) {
        setMyLawyer(matchedLawyer[0]?.providerId);
      } else {
        setMyLawyer(6);
      }
    }
  }, [lawyers.data, user.userId]);


  console.log("myLawyer",myLawyer);
  const id = myLawyer;

  const { lawyer, lawyerError } = useGetLawyer(id);

  console.log("hii",lawyer)

  useEffect(() => {
    if (lawyer && lawyer.data) {
      setchartedAccountantData(lawyer.data);
    }
  }, [lawyer]);
        ...
        <Stack sx={{ maxWidth: 720, mx: 'auto' }}>
          <Stack direction="column" alignItems="start" spacing={2}>
            ...
            <Stack direction="row" alignItems="center" sx={{ marginTop: '10px' }}>
              ...
              <Typography sx={{ ml: 1 }}>{chartedAccountantData?.providerName}</Typography>
            </Stack>
            <Stack direction="row" alignItems="center" sx={{ marginTop: '10px' }}>
              <Typography sx={{ mr: 1, minWidth: 180, fontWeight: 'bold' }}>Experience Level:</Typography>
              <Typography sx={{ ml: 1 }}>{chartedAccountantData?.experienceLevel}</Typography>
            </Stack>
            <Stack direction="row" alignItems="center" sx={{ marginTop: '10px' }}>
            ...
            <Grid item xs={12} md={9}>
              <Card sx={{ borderRadius: 2, boxShadow: 3 }}>
                <CardContent>
                  ...
                  <Typography>Name: {chartedAccountantData.providerName}</Typography>
                  <Typography>Experience Level: {chartedAccountantData.experienceLevel}</Typography>
