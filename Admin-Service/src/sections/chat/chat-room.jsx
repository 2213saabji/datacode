...
export default function ChatRoom({ participants, conversationCreatedBy, conversation, userId }) {
  ...
  const privateParticipant = participants?.filter((participant) => participant.userId !== userId);
  ...
  const renderContent = (
    <>
      {group ? (
        <ChatRoomGroup participants={participants} conversationCreatedBy={conversationCreatedBy} />
      ) : (
        <ChatRoomSingle
          participant={privateParticipant[0]}
          userId={userId}
