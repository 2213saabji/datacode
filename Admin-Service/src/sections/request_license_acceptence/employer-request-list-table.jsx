...
import {
  RenderCellEmployerId,
  RenderCellBusinessName,
  RenderCellEmployerName,
...
export default function EmployerRequestListView({ currentTab }) {
  ...
  // ----------Doctor details-----------
  const { users: employerListData, usersLoading: employerusersLoading } = useGetEmployerRequestList(
    user.accessToken
  );
  ...
  const handleVerifyEmployerDetails = useCallback(
    async (registrationNo) => {
      try {
        const filteredDoctors = dataFiltered.filter(
          (employer) => employer.registrationNumber === registrationNo
        );
        const DataToSend = {
          employerName: filteredDoctors[0]?.employerName,
          ...
          registrationNumber: filteredDoctors[0]?.registrationNumber,
          registrationFrontImageUrl: filteredDoctors[0]?.registrationImageUrl?.front?.preview,
          registrationBackImageUrl: filteredDoctors[0]?.registrationImageUrl?.back?.preview,
        };
  ...
  const handleAcceptEmployerDetails = useCallback(
    async (userId, registrationNo, Token) => {
      try {
        const DataToSend = {
          verificationToken: Token,
          registrationNumber: registrationNo,
        };
        const response = await SendAcceptRequest(DataToSend, user.accessToken, 'employer');
        if (response.success) {
          const newData = {
            userRoleId: 30,
          };
  ...
  const handleEmployerRejectRequest = useCallback(
    async (id) => {
      try {
        // Use the correct API endpoint URL
        const apiUrl = `${endpoints.requestLicenseAcceptance.EmployerRejectRequest}/${id}`;
  ...
  const columns = [
    {
      ...
      flex: 1,
      minWidth: 50,
      renderCell: (params) => <RenderCellEmployerId params={params} />,
    },
    {
      field: 'EmployerName',
      ...
      flex: 1,
      minWidth: 100,
      renderCell: (params) => <RenderCellEmployerName params={params} />,
    },
    {
      field: 'businessName',
      ...
      flex: 1,
      minWidth: 100,
      renderCell: (params) => <RenderCellBusinessName params={params} />,
    },
    {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <RenderCellVerificationButton
          ...
          handleVerifyEmployerDetails={handleVerifyEmployerDetails}
        />
      ),
    },
    {
      ...
      renderCell: (params) => (
        <RenderCellAcceptRequestButton
          params={params}
          currentTab={currentTab}
          handleAcceptEmployerDetails={handleAcceptEmployerDetails}
          handleEmployerDeleteRow={handleEmployerDeleteRow}
          handleEmployerRejectRequest={handleEmployerRejectRequest}
        />
      ),
    },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label="View Details"
          onClick={() => handleViewRow(params.row.employerId)}
        />,
        // <GridActionsCellItem
        //   showInMenu
...
EmployerRequestListView.propTypes = {
