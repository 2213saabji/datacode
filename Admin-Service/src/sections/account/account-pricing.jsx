...
import AccountPricingCard from './pricing-cards';
...
export default function AccountPricing() {
  return (
    <Container
      sx={{
        pt: 2,
    ...
    >
      <Stack spacing={2} sx={{ p: 8, backgroundColor: '#97C4EF' , borderRadius: 2}}>
        <Typography variant="h3" align="center" sx={{ mb: 2, color: '#010104'}}>
          Flexible plans for your
          <br /> {`community's size and needs`}
        </Typography>

        <Typography align="center" sx={{ color: '#010104', }}>
          Choose your plan and make modern online conversation magic
        </Typography>
      </Stack>

      <Box sx={{ mt: 9, mb: 5, position: 'relative', backgroundColor: '#97C4EF', borderRadius: 4}}>
      ...
      >
        {_pricingPlans.map((card, index) => (
          <AccountPricingCard key={card.subscription} card={card} index={index} />
        ))}
      </Box>
    </Container>
  );
