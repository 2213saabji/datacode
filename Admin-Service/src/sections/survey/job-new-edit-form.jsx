...
export default function JobNewEditForm({ surveyData }) {
  ...
  console.log(filteredVoterListArr);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const response = await CreateSurvey(data, user?.userId);
      if (response) {
        // console.log("data-->",response)
        value.userIdsForSurvey?.forEach((id) => {
          createNotifications(
            id,
            'Survey created by admin',
  ...
  const renderProperties = (
    <Card>
      ...
      <Stack spacing={3} sx={{ p: 3 }}>
        ...
        <Stack spacing={1.5}>
          <Typography variant="subtitle2" name="surveyQuestions">
            Survey Questions :
          </Typography>

          {value?.surveyQuestions && value?.surveyQuestions.length > 0 ? (
            <QuestionsList
              title="Tasks"
              handleOpen={handleOpen}
              list={value?.surveyQuestions}
          ...
          {surveyData?.surveyQuestionsBanks && surveyData?.surveyQuestionsBanks.length > 0 && (
            <QuestionsList
              title="Tasks"
              handleOpen={handleOpen}
              list={surveyData?.surveyQuestionsBanks}
              setupdateque={setupdateque}
            />
          )}
        </Stack>
