...
import { useGetTripManaged } from 'src/api/trip'; // Changed to useGetTrip
...
import AmbulanceDetailsHero from '../ambulance-details-hero'; // Changed to TripDetailsHero
import MapWithPolyline from '../googleMapComponents/mapWithPolyline';
// ----------------------------------------------------------------------

export default function AmbulanceManagedDetailsView({ id }) {
  const { user } = useAuthContext();
  // Changed function name to TripDetailsView
  const { trip, tripError } = useGetTripManaged(id, user?.accessToken); // Changed to useGetTrip
  console.log(trip);
  ...
  const renderPost = trip && ( // Changed to trip
    <>
      <AmbulanceDetailsHero title="TRIP DETAILS" coverUrl={BannerBlurImg} />
          ...
          links={[
            {
              ...
              href: paths.dashboard.callAmbulance.managedList, // Changed to paths.dashboard.trip.root
            },
            {
              name: 'Details',
              href: paths.trip, // Changed to paths.trip
...
AmbulanceManagedDetailsView.propTypes = {
  id: PropTypes.string,
