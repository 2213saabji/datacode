...
import { createVendorProfile, updateVendorProfile } from 'src/api/vendor';
...
export default function VendorNewEditForm({ currentDriver }) {
  ...
  const vendorId = currentDriver?.data.providerId;
  ...
  const ServiceData = ServiceListArr.filter((list) => list.userRoleId === 32) // Filter out services with userRoleId of 31
  .map((list) => ({
  ...
  const usersListArr = useMemo(
    () =>
      userList?.users
        ? userList.users.filter((user) => user.userRoleId === 32 && user.UserProfile)
        : [],
    [userList]
  );
  ...
  const onSubmitProfile = handleSubmit(async (data) => {
    try {
      console.log('>>>', data);
      const response = await createVendorProfile(data);

      if (response) {
        enqueueSnackbar('Vendor Profile created successfully', { variant: 'success' });
        navigate('/dashboard/lms_vendor');
      } else {
        enqueueSnackbar('Failed to create Vendor profile', { variant: 'error' });
      }
    } catch (error) {
      // Handle errors here if necessary
      console.error('Error submitting Vendor profile:', error);
      enqueueSnackbar('An error occurred while creating  Vendor profile', { variant: 'error' });
    }
  });
  ...
  const onSubmitProfileUpdate = handleSubmit(async (data) => {
    try {
      const response = await updateVendorProfile(vendorId, data);
      console.log('>>>>updatex', response);
      if (response) {
        enqueueSnackbar(' Vendor Profile Signature created successfully', { variant: 'success' });
        navigate(`/dashboard/vendor/${vendorId}`);
      } else {
        enqueueSnackbar('Failed to create Vendor profile Signature', { variant: 'error' });
      }
    } catch (error) {
      // Handle errors here if necessary
      console.error('Error submitting Vendor profile Signature:', error);
      enqueueSnackbar('An error occurred while creating  Vendor profile Signature', {
        variant: 'error',
      });
    }
  });
          ...
          <Grid container spacing={3}>
            <Grid xs={12} md={8}>
              <Card sx={{ p: 3 }}>
                ...
                <Stack direction="row" alignItems="center" spacing={50} sx={{ mt: 3 }}>
                  <Button
                    variant="contained"
                  ...
                  <LoadingButton type="submit" variant="contained">
                    {!currentDriver ? 'Create Vendor' : 'Save Changes'}
                  </LoadingButton>
                </Stack>
              </Card>
            </Grid>
