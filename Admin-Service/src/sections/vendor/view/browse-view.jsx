...
export default function BrowseView() {
  ...
  const ServiceData = ServiceListArr.filter((list) => list.userRoleId === 32).map((list) => ({
  ...
  const handleViewNext = useCallback(
    () => {
      router.push(paths.dashboard.lms_vendor.filter);
    },
    [router]
  );
    ...
    <>
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'} sx={{ py: 4 }}>
        <CustomBreadcrumbs
          ...
          links={[
            {
              name: 'FOR WHICH YOU ARE LOOKLING A VENDOR !',
            },
          ]}
          sx={{
            mb: { xs: 3, md: 5 },
