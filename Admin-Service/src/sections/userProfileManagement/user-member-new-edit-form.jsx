...
import { useGetUsers, UserRoleGetter } from 'src/api/user';
...
export default function MemberNewEditForm({ currentVoter }) {
  ...
  const usersListArr =
    usersList && usersList.data
      ? usersList.data.filter((userss) => !VoterProfileList.includes(userss.userId))
      : [];
  ...
  const defaultValuesUserDetailsPhase1 = {
    phone: '',
    firstName: '',
    lastName: '',
    fatherName: '',
  ...
  // for Profile from
  const {
    handleSubmit: handleSubmitUserDetailsPhase0,
    reset: resetUserDetailsPhase0,
    watch: UserDetailsPhase0Watch,
    setValue: setresetUserDetailsPhase0Value,
  ...
  const resetUserDetailsPhase0values = UserDetailsPhase0Watch();
  ...
  const onSubmitUserDetailsPhase0 = handleSubmitUserDetailsPhase0(async (data) => {
    try {
      if (bool) {
        ...
        setresetUserDetailsPhase0Value('RoleId', data?.RoleId);
        setNewVoterDetailsPhase({ Phase0: false, Phase1: true, Phase2: false, Phase3: false });
      }
    } catch (error) {
      console.error(error);
    ...
    try {
      if (bool) {
        ...
        if (
          ...
          setNewVoterDetailsPhase({ Phase0: false, Phase1: false, Phase2: true, Phase3: false });
        }
      }
    } catch (error) {
      console.error(error);
  ...
  const onSubmitUserDetailsPhase2 = handleSubmitUserDetailsPhase2(async (data) => {
    try {
      setNewVoterDetailsPhase({ Phase0: false, Phase1: false, Phase2: false, Phase3: true });
    } catch (error) {
      console.error(error);
      enqueueSnackbar('Something went wrong.', { variant: 'error' });
    }
  ...
  const onSubmitPassword = handleSubmitPassword(async (data) => {
    try {
      const datatoSend = {
        ...
        userRoleId: resetUserDetailsPhase0values?.RoleId,
      ...
      if (response === 'ok') {
        ...
        setNewVoterDetailsPhase({ Phase0: true, Phase1: false, Phase2: false, Phase3: false });
  ...
  function roleShowFunction(e) {
    ...
    setresetUserDetailsPhase0Value('productId', productId);
    setresetUserDetailsPhase0Value('RoleId', '');
    setUserRoleId([]);
            ...
            {newVoterDetailsPhase.Phase0 && (
              <FormProvider methods={userDetailsMethodsPhase0} onSubmit={onSubmitUserDetailsPhase0}>
                ...
                <RHFAutocomplete
                  ...
                  label={
                    <span>
                      Current your Role<span style={{ color: 'red' }}> *</span>
                    </span>
                  }
