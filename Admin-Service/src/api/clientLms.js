...
export function useGetClient(clientId) {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = clientId ? `${endpoints.client.details}/${clientId}` : null;
...
export async function createDriverProfile(formData) {
  const URL =  ATTPL_LMS_HOST_API+endpoints.client.create;
  const accessToken = localStorage.getItem(STORAGE_KEY);
  console.log(URL,">>>>>>>>>>>>>");
...
export async function updateDriverProfile(clientCompanyId, formData) {
  const URL = `${ATTPL_LMS_HOST_API+endpoints.client.update}/${clientCompanyId}`;
