...
import { useGetDesires } from 'src/api/desire';
...
import AppointmentTableFiltersResult from '../Desire-table-filters-result';
import AppointmentManagementAnalytics from '../Desire_management-analytics';
import {
  RenderCellFoundationYear,
  RenderCellFoundationDate,
  RenderCellAppointmentName,
  RenderCellAppointmentLeader,
} from '../Desire-table-row';
...
export default function DesireListView() {
  ...
  const { Desires, DesiresLoading } = useGetDesires();

  const sortedData = Desires?.data?.sort((a, b) => {
  ...
  useEffect(() => {
    if (sortedData) {
      if (user.userRoleType === 'Voter') {
        ...
        console.log(`first data----> ${filteredAppointments}`);
        console.log(status);
        ...
        console.log(`Candidate data----> ${sortedData}`);
        const filteredAppointments = sortedData?.filter(
          (appointmen) => appointmen.candidateId === user.userId
        );
        console.log(`Candi data----> ${filteredAppointments}`);
        setTableData(filteredAppointments);
      }
    }
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Desire.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Desire.details(id));
    },
    [router]
  );
      ...
      getActions: (params) => {
        const actions = [
          <GridActionsCellItem
            ...
            label="View"
            onClick={() => handleViewRow(params.row.desireAppointmentId)}
          />,
        ];
        ...
        if (p) {
          actions.push(
            <GridActionsCellItem
              ...
              label="Edit"
              onClick={() => handleEditRow(params.row.desireAppointmentId)}
            />
          );
        }
        return actions;
    ...
    <>
      ...
      >
        ...
        <DataGrid
          ...
          rows={dataFiltered}
          columns={columns}
          getRowId={(row) => row.desireAppointmentId}
          loading={DesiresLoading}
          getRowHeight={() => 'auto'}
