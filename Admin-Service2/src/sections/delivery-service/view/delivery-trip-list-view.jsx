...
import { useGetDeliveryTrips } from 'src/api/delivery_service';
...
import DeliveryTripTableFiltersResult from '../delivery-trip-table-filters-result';
...
export default function DeliveryTripListView() {
  ...
  useEffect(() => {
    if (data) {
      // Changed to trips
      const updatedTableData = data.map((booking) => ({
        // Changed to trips
        ...booking, // Changed to trip
        id: booking.bookingId, // Changed to tripId
      }));
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.deliveryService.details(id)); // Changed to trip
    },
    [router]
  );
  ...
  const columns = [
    ...
    {
      field: 'bookingId',
      headerName: 'Booking Id', // Changed to Trip
      flex: 1,
      minWidth: 40,
      hideable: false,
    ...
    },
    {
      ...
      flex: 1,
      editable: true,
      renderCell: (params) => <Typography>{params.row?.DriverDetail?.driverName}</Typography>,
    },
    {
      field: 'vehicleType',
      ...
      width: 150,
      renderCell: (params) => (
        <Typography>{params.row?.DriverDetail?.Vehicles?.vehicleType}</Typography>
      ),
    },
    {
      field: 'itemName',
      ...
      width: 150,
      renderCell: (params) => <Typography>{params.row?.itemDetails?.itemName}</Typography>,
    },
    {
      field: 'itemWeight',
      headerName: 'Item Weight',
      width: 150,
      renderCell: (params) => <Typography>{params.row?.itemDetails?.weight}</Typography>,
    },
    {
      field: 'bookingStatus',
  ...
  return (
    <>
      ...
      >
        <CustomBreadcrumbs
          heading="DELIVERY BOOKING LIST" // Changed to Trip List
          links={[
            {
              name: 'Delivery Booking', // Changed to Trip
