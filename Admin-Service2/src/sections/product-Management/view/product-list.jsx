...
import { RenderCellProductType, RenderCellProductdescription } from '../product-row-table';
...
export default function ProductListView() {
  ...
  const { users, usersLoading } = useGetProductRolesList(user.accessToken);
  ...
  // console.log("utut",customRoleType);

  useEffect(() => {
    if (customRoleType) {
      const updatedTableData = customRoleType.map((userItem) => ({
        ...userItem,
        id: userItem.productId,
      }));
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `/user/delete/product-detail/${id}`;
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.productManagement.edit(id));
    },
    [router]
  );
  ...
  const columns = [
    ...
    {
      field: 'userProductDescription',
      headerName: 'Product Type',
      flex: 1,
      width: 500,
      renderCell: (params) => <RenderCellProductdescription params={params} />,
    },
    {
      type: 'actions',
      field: 'actions',
      ...
      >
        ...
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Product',
              // href: paths.dashboard.productManagement.root,
            },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.productManagement.new}
              variant="contained"
