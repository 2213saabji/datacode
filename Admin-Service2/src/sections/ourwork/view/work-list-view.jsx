...
import { deleter } from 'src/utils/axios-work';

import { useGetWork } from 'src/api/work';
...
import CategoryTableFiltersResult from '../work-table-filters-result';
import { RenderCellClaimDate, RenderCellClaimAmount } from '../work-table-row';
...
export default function WorkListView() {
  ...
  const { works, worksLoading } = useGetWork();
  ...
  useEffect(() => {
    if (works && works.data) {
      ...
      const updatedTableData = works.data.map((claim) => ({
        ...claim,
        id: claim.ourWorkId,
      }));
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.work.edit(id));
      // console.log(id);
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.work.details(id));
    },
    [router]
  );
