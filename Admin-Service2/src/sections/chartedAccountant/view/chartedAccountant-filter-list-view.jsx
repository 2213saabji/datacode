...
export default function LawyerFilterListView() {
  ...
  console.log('filter', charteredaccountants);
  ...
  const charteredaccountantsData = useMemo(
    () => charteredaccountants?.data?.filter((chartedAccountant) => chartedAccountant.User.userRoleId === 49),
    [charteredaccountants]
  );
  // const charteredaccountantsData = charteredaccountants?.data
  useEffect(() => {
    if (charteredaccountantsData) {
      const updatedTableData = charteredaccountantsData.map((charteredaccountant) => ({
        ...charteredaccountant,
        id: charteredaccountant.providerId,
      }));
      setTableData(updatedTableData);
    }
  }, [charteredaccountantsData]);
  //   useEffect(() => {
  ...
  const handleBookLawyer = async (id, userId, providerId) => {
    try {
      
      console.log(`Book lawyer with ID: ${userId}`);
      ...
      const Url = `/LMS_case/lawyer_list`;
      const status = 'unread';
    ...
    // New column for the button
    {
      ...
      renderCell: (params) => (
        <Button
          ...
          onClick={() => handleBookLawyer(params.row.id, params.row.userId, params.row.providerId)}
        >
          Book CA
        </Button>
      ),
  ...
  return (
    <>
      ...
      >
        <CustomBreadcrumbs
          heading="AVAILABLE CHARTED ACCOUNTANT LIST"
          links={[
            {
              name: 'CA',
...
function applyFilter({ inputData }) {
  ...
  return inputData.filter((item) => item?.serviceId.toString() === serviceId || item?.issueId.toString() === issueId);
}
