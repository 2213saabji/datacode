...
import { createDriverProfile, updateDriverProfile } from 'src/api/chartedAccountant';
import {SERVICE_AREA, TEHSIL_OPTIONS ,VILLAGE_OPTIONS,DISTRICT_OPTIONS,SERVICE_PROVIDER_TYPE} from 'src/_mock/_service';
...
export default function ChartedAccountantEditForm({ currentDriver }) {
  ...
  const chartedAccountId = currentDriver?.data.providerId;
  console.log(chartedAccountId, '>>>>');
  ...
  const ServiceData = ServiceListArr.filter((list) => list.userRoleId === 49) // Filter out services with userRoleId of 31
  .map((list) => ({
  ...
  const usersListArr = useMemo(
    () =>
      userList?.users
        ? userList.users.filter((user) => user.userRoleId === 49 && user.UserProfile)
        : [],
    [userList]
  );
  ...
  const onSubmitProfile = handleSubmit(async (data) => {
    try {
      ...
      if (response) {
        ...
        navigate('/dashboard/chartered_accountant');
      } else {
        enqueueSnackbar('Failed to create Charted Account profile', { variant: 'error' });
      }
    } catch (error) {
      // Handle errors here if necessary
      console.error('Error submitting Charted Account profile:', error);
      enqueueSnackbar('An error occurred while creating  Charted Account profile', {
        variant: 'error',
      });
    }
  });
  ...
  const onSubmitProfileUpdate = handleSubmit(async (data) => {
    try {
      const response = await updateDriverProfile(chartedAccountId, data);
      console.log('>>>>updatex', response);
      if (response) {
        enqueueSnackbar(' Charted Account Profile Signature created successfully', {
          variant: 'success',
        });
        // navigate('/dashboard/driver');
        navigate(`/dashboard/chartered_accountant/${chartedAccountId}`);
      } else {
        enqueueSnackbar('Failed to create Charted Account profile ', { variant: 'error' });
      }
    } catch (error) {
      // Handle errors here if necessary
      console.error('Error submitting Charted Account profile Signature:', error);
      enqueueSnackbar('An error occurred while creating  Charted Account profile Signature', {
        variant: 'error',
      });
    }
  });
              ...
              <Card sx={{ p: 3 }}>
                ...
                <Stack direction="row" alignItems="center" spacing={2} sx={{ mt: 3 }}>
                  ...
                  <LoadingButton
                    ...
                    onClick={
                      !currentDriver
                      // ? () => navigate('/dashboard/LMSUserProfile/new')
                      // : handleImageUpload
                    }
                    loading={false}
                  >
                    {!currentDriver ? 'Create ChartedAccountant' : 'Save Changes'}
                  </LoadingButton>
                </Stack>
              </Card>
            </Grid>
...
ChartedAccountantEditForm.propTypes = {
