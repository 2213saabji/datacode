...
import {
  ...
  RenderCellBusinessmanId,
  RenderCellLicenseNumber,
  RenderCellBusinessmanName,
...
export default function BusinessmanRequestListView({ currentTab }) {
  ...
  // ----------Doctor details-----------
  const { users: BusinessmanListData, usersLoading: BusinessmanusersLoading } =
    useGetBusinessmanRequestList(user.accessToken);
  const customRoleType = useMemo(
    () =>
      BusinessmanListData && BusinessmanListData.data
        ? BusinessmanListData.data.filter((role) => true)
        : [],
    [BusinessmanListData]
  );
  ...
  const handleBusinessmanRejectRequest = useCallback(
    async (id) => {
      try {
        // Use the correct API endpoint URL
        const apiUrl = `${endpoints.requestLicenseAcceptance.BusinessmanRejectRequest}/${id}`;
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.request_license_acceptence.businessman_details(id));
    },
    [router]
  );
  ...
  const columns = [
    {
      field: 'BusinessmanId',
      headerName: 'Businessman Id',
      flex: 1,
      minWidth: 50,
      renderCell: (params) => <RenderCellBusinessmanId params={params} />,
    },
    {
      field: 'BusinessmanName',
      headerName: 'Businessman Name',
      flex: 1,
      minWidth: 100,
      renderCell: (params) => <RenderCellBusinessmanName params={params} />,
    },
    {
      field: 'businessName',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <RenderCellVerificationButton
          ...
          handleVerifyBusinessmanDetails={handleVerifyBusinessmanDetails}
        />
      ),
    },
    {
      ...
      renderCell: (params) => (
        <RenderCellAcceptRequestButton
          params={params}
          currentTab={currentTab}
          handleAcceptBusinessmanDetails={handleAcceptBusinessmanDetails}
          handleBusinessmanDeleteRow={handleBusinessmanDeleteRow}
          handleBusinessmanRejectRequest={handleBusinessmanRejectRequest}
        />
      ),
    },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label="View Details"
          onClick={() => handleViewRow(params.row.businessmanId)}
        />,
        // <GridActionsCellItem
        //   showInMenu
      ...
      >
        ...
        >
          <DataGrid
            ...
            rows={dataFiltered}
            columns={columns}
            loading={BusinessmanusersLoading}
            getRowHeight={() => 'auto'}
...
BusinessmanRequestListView.propTypes = {
