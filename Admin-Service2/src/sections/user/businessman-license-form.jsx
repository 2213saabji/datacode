...
import { RequestBusinessmanForm } from 'src/api/requestLicenseAcceptance';
...
export default function BusinessmanLicenseForm() {
  ...
  const ProfileSchema = Yup.object().shape({
    businessmanName: Yup.string().required('Businessman Name is required'),
  ...
  const defaultValues = useMemo(
    () => ({
      businessmanName: `${userdata?.UserProfile?.firstName}` || '',
      phoneNumber: '',
      emailId: '',
      businessName: '',
      businessType: '',
  ...
  const onSubmitBusinessmanupload = handleSubmitProfile(async (data) => {
    try {
      ...
      const response = await RequestBusinessmanForm(newdata, userdata.accessToken);
      if (response) {
        resetProfile();
        setErrorState(false);
  ...
  return (
    <FormProvider
      ...
      onSubmit={user?.UserProfile && onSubmitBusinessmanupload}
    >
      <Grid sx={{ mb: 4, mt: 4 }}>
        <Grid xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            <CardHeader title="BUSINESSMAN VERIFICATION FORM" sx={{ pl: 0, mb: 3 }} />
            <Box sx={{ mb: 2 }}>
              <Box sx={{ mb: 2 }}>
                <Tabs
                  value={currentTab}
            ...
            >
              <RHFTextField
                ...
                label={
                  <span>
                    Businessman Name<span style={{ color: 'red' }}> *</span>
                  </span>
                }
              />
              <RHFTextField
                ...
                label={
                  <span>
                    Businessman Phone No.<span style={{ color: 'red' }}> *</span>
                  </span>
                }
              />
              <RHFTextField
...
BusinessmanLicenseForm.propTypes = {};
