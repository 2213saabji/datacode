...
export default function JobFiltersResult({
  ...
  const handleRemoveJobTypes = (inputValue) => {
    const newValue = filters.jobTypes.filter((item) => item !== inputValue);
    ...
    <Stack spacing={1.5} {...other}>
      ...
      <Stack flexGrow={1} spacing={1} direction="row" flexWrap="wrap" alignItems="center">
        ...
        {!!filters.jobTypes.length && (
          <Block label="Job Types:">
            {filters.jobTypes.map((item) => (
              <Chip
                key={item}
                label={item}
                size="small"
                onDelete={() => handleRemoveJobTypes(item)}
              />
            ))}
          </Block>
        )}
