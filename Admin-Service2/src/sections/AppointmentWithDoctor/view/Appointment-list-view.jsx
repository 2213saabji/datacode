...
import { useGetdoctorAppointments } from 'src/api/appointmentwithdoctor';
...
export default function AppointmentwithdoctorListView() {
  ...
  const { doctorappointments, doctorappointmentsLoading } = useGetdoctorAppointments();
  const sortedData = doctorappointments?.data?.sort((a, b) => {
  ...
  const p = user.userRoleType === 'Doctor';
    ...
    if (sortedData) {
      if (user.userRoleType === 'Voter') {
        const filteredAppointments = sortedData.filter(
          (appointmen) => appointmen.patientId === user.userId
        );
        console.log(filteredAppointments);
        const filter = filteredAppointments.filter((i) => i.appointmentPassStatus === status);
        setTableData(filter);
        console.log(filter);

        // setTableData( sortedData.data);
        // in else if block need to edit the code from Admin to Candidate 6/7/2024
      } else if (user.userRoleType === 'Doctor') {
        const filteredAppointments = sortedData.filter(
          (appointmen) => appointmen.doctorId === user.userId
        );
        setTableData(filteredAppointments);
      }
    }
  }, [sortedData, status, user.userId, user.userRoleType]);

  // useEffect(() => {

  //   if (  sortedData&&  sortedData.data) {
  //     if (user.userRoleType === 'Voter') {
  //       const filteredAppointments =  sortedData..filter(
  //         (appointmen) => appointmen.patientId=== user.userId
  //       );
  ...
  //       // in else if block need to edit the code from Admin to Candidate 6/7/2024
  //     } else if (user.userRoleType === 'Doctor') {
  //       const filteredAppointments =  sortedData.data.filter(
  //         (appointmen) => appointmen.doctorId === user.userId
  //       );
  //       setTableData(filteredAppointments);
  //     }
  ...
  const inProgress = tableData.filter((i) => i.appointmentStatus === 'in-progress');
  const close = tableData.filter((i) => i.appointmentStatus === 'close');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Appointmenttodoctor.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.Appointmenttodoctor.details(id));
    },
    [router]
  );
      ...
      getActions: (params) => {
        const actions = [
          <GridActionsCellItem
            ...
            label="View"
            onClick={() => handleViewRow(params.row.doctorAppointmentId)}
          />,
        ];
        ...
        if (p) {
          actions.push(
            <GridActionsCellItem
              ...
              label="Edit"
              onClick={() => handleEditRow(params.row.doctorAppointmentId)}
            />
          );
        }
        return actions;
  ...
  return (
    <>
      ...
      >
        <CustomBreadcrumbs
          heading="DOCTOR APPOINTMENT LIST "
          links={[
            {
              name: 'Dashboard',
        ...
        <DataGrid
          ...
          rows={dataFiltered}
          columns={columns}
          getRowId={(row) => row.doctorAppointmentId}
          loading={doctorappointmentsLoading}
          getRowHeight={() => 'auto'}
                ...
                <GridToolbarContainer>
                  {!p && (
                    <Button
                      ...
                      to="/dashboard/Appointmenttodoctor/card"
                      variant="outlined"
                      color="primary"
