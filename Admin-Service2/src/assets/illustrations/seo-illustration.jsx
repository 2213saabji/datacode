...
import { useGetAllList } from 'src/api/election_details';
...
// const cmImage = [
//   {
//     path: '/assets/images/ALLCM/Maharashtra.webp',
//   },
//   {
//     path: "/assets/images/ALLCM/Gujarat.webp",
//   },
// ]
...
function SeoIllustration({ ...other }) {
  ...
  const { CM_Oppositions } = useGetAllList(user?.accessToken)
 
  const userState = user?.UserAddressesses?.[0]?.userState;
 
  const filteredData = CM_Oppositions?.data?.filter(data => data.state === userState)
 
  const cmImage = filteredData?.[0]
  const oppositonImage = filteredData?.[0]
  // console.log('----->', oppositonImage)
  ...
  return (
    <Box
      sx={{ display: 'flex', justifyContent: 'center', flexDirection: { xs: 'row', md: 'column' }, gap: 2 }}
    >
      {/* {cmImage.map((item, key) => ( */}
      {cmImage?.cmImage?.url && <Box sx={{ position: 'relative', display: 'flex', }}>
        <Image
          alt="CM"
          src={cmImage?.cmImage?.url}
          width={{ xs: 140, sm: 250, md: 250 }}
          height={{ xs: 140, sm: 200, md: 200 }}
        />
        <Typography variant="h6" sx={{ position: 'absolute', fontSize: { xs: '12px', sm: '15px', md: '17px' }, bottom: '10px', left: '10px',color: 'text.primary' , zIndex: '1' }}>{cmImage?.cmName}</Typography>
        ...
        <Box
          sx={{
            ...
            ...bgGradient({
              direction: 'to top',
              startColor: `${alpha(theme.palette.primary.main, 0.8)} 0%`, // Primary color with some transparency
              endColor: `${alpha(theme.palette.primary.main, 0)} 30%`,
            }),
          }}
        />
      </Box>}
 
      {oppositonImage?.oppositionImage?.url && <Box sx={{ position: 'relative', display: 'flex', }}>
 
        <Image
          alt="Oppositon Image not available"
          src={oppositonImage?.oppositionImage?.url}
          width={{ xs: 140, sm: 250, md: 250 }}
          height={{ xs: 140, sm: 200, md: 200 }}
        />
        <Typography variant="h6" sx={{ position: 'absolute', fontSize: { xs: '12px', sm: '15px', md: '17px' }, bottom: '10px', left: '10px',color: 'text.primary' , zIndex: '1' }}>{oppositonImage?.oppositionName}</Typography>
