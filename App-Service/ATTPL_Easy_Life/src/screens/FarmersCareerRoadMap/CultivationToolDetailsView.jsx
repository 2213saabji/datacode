...
import {fetchCultivationTool} from '../../redux/slices/CRM/FarmerCareerRoad/FarmerTools/CultivationToolSlice';

export default function CultivationToolDetailsView({route}) {
  ...
  const cultivationEquipmentId = route?.params?.cultivationEquipmentId;
  ...
  const getData = async () => {
    try {
      const result = await dispatch(
        fetchCultivationTool(cultivationEquipmentId),
      );
      if (fetchCultivationTool.fulfilled.match(result)) {
        if (result.payload) {
        ...
        console.log(result.payload || 'Failed to fetch cultivation tool.');
      }
    } catch (error) {
  ...
  const renderContent = (
    <Card
      ...
      <View style={styles.section}>
        <Text style={[styles.subtitle, {color: colors.text}]}>Depth</Text>
        <Text style={{color: colors.text, fontSize: 18, fontWeight: 'bold'}}>
          {jobData?.depthCm} cm
        </Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <Text style={{color: colors.text, fontSize: 18, fontWeight: 'bold'}}>
          {jobData?.workingWidthMeters} m
        </Text>
      </View>

      <View style={styles.section}>
        <Text style={[styles.subtitle, {color: colors.text}]}>Size</Text>
        <Text style={{color: colors.text, fontSize: 18, fontWeight: 'bold'}}>
          {jobData?.size}
        </Text>
      </View>
