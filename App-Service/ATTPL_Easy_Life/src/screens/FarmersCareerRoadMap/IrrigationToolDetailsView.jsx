...
import {fetchIrrigationTool} from '../../redux/slices/CRM/FarmerCareerRoad/FarmerTools/IrrigationToolSlice';

export default function IrrigationToolDetailsView({route}) {
  ...
  const irrigationSystemId = route?.params?.irrigationSystemId;
  ...
  const getData = async () => {
    try {
      const result = await dispatch(fetchIrrigationTool(irrigationSystemId));
      if (fetchIrrigationTool.fulfilled.match(result)) {
        if (result.payload) {
        ...
        console.log(result.payload || 'Failed to fetch Irrigation tool.');
      }
    } catch (error) {
  ...
  const renderContent = (
    <Card
      ...
      <View style={styles.section}>
        ...
        <Text style={{color: colors.text, fontSize: 18, fontWeight: 'bold'}}>
          {jobData?.coverageAreaHectares} hectare
        </Text>
      </View>

      <View style={styles.section}>
        <Text style={[styles.subtitle, {color: colors.text}]}>Nozzle Type</Text>
        <Text style={{color: colors.text, fontSize: 18, fontWeight: 'bold'}}>
          {jobData?.nozzleType}
        </Text>
      </View>
