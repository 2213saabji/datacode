...
import {fetchAppointment} from '../../../redux/slices/CRM/FarmerCareerRoad/FarmerCareerRoadSlice';

export default function FarmerAppointmentDetailsView({route}) {
  ...
  const agricultureAppointmentId = route?.params?.agricultureAppointmentId;
  ...
  const getData = async () => {
    try {
      const result = await dispatch(fetchAppointment(agricultureAppointmentId));
