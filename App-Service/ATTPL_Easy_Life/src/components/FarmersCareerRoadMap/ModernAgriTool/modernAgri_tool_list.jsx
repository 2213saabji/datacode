...
import AgriToolItem from './AgriToolItem';
...
import {
  fetchModernAgriTools,
  deleteModernAgriTool,
} from '../../../redux/slices/CRM/FarmerCareerRoad/FarmerTools/ModernAgriToolSlice';
import LoadingListView from '../../ReusableComp/LoadingListView';

const ModernAgriToolList = ({ navigation }) => {
  ...
  const fetchTools = async () => {
    try {
      const result = await dispatch(fetchModernAgriTools());
      if (fetchModernAgriTools.fulfilled.match(result)) {
        if (result.payload) {
          setLoading(false);
  ...
  const deleteJob = useCallback(
    async appointmentId => {
      try {
        const result = await dispatch(deleteModernAgriTool(appointmentId));
        if (deleteModernAgriTool.fulfilled.match(result)) {
          if (result.payload) {
    ...
    [dispatch, showAlert, deleteModernAgriTool],
  );
    ...
    <>
      <SafeAreaView
        ...
        <ModernSortBar
          ...
          buttonName="Modern Tool"
          screenName="ModernToolCreate"
...
export default ModernAgriToolList;
