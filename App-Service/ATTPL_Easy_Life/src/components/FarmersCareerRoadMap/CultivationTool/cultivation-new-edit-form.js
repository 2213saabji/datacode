...
} from '../../../_mock/farmer_cultivation_tool';
import {useNavigation} from '@react-navigation/native';
import {color} from 'react-native-elements/dist/helpers';

export default function CultivationNewEditForm({
  jobData,
  currentTool,
  currentCultivation,
  fetchJobs,
  ...
  const PartySchema = Yup.object().shape({
    ...
    workingWidthMeters: Yup.number().required(
      'Working Width Meters is required',
    ),
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      price: currentCultivation?.data.price || 0,
      brand: currentCultivation?.data.brand || '',
      workingWidthMeters: currentCultivation?.data.workingWidthMeters || 0,
      image1: currentCultivation?.data?.imageUrl?.image1 || '',
      image2: currentCultivation?.data?.imageUrl?.image2 || '',
      depthCm: currentCultivation?.data.depthCm || 0,
    }),
    [currentTool],
  );
  ...
  const onSubmit = handleSubmit(async data => {
    ...
    const result = await dispatch(createCultivationTool(updatedData));
    // console.log('NEW result======================>>>>>>>>>>>>', result);
    if (createCultivationTool.fulfilled.match(result)) {
      showAlert('Tool Added Successfully', 'success');
      navigation.replace('CultivationTool');
    } else {
  ...
  return (
    <View
        ...
        <Controller
          ...
          render={({field: {onChange, value}, fieldState: {error}}) => (
            <SingleDropdownComponent
              options={cultivationEquipmentTypes}
              value={value}
              setValue={onChange}
      ...
      <View>
        <Text style={[styles.title, {color: colors.text}]}>Tool Size</Text>
        <Controller
          name="size"
          control={control}
          render={({field: {onChange, value}, fieldState: {error}}) => (
            <SingleDropdownComponent
              options={cultivationEquipSize}
              value={value}
              setValue={onChange}
              containerColor="transparent"
              inputBorderRadius={6}
      ...
      <View>
        <Text style={[styles.title, {color: colors.text}]}>Working Width</Text>
      ...
      <View>
        <Text style={[styles.title, {color: colors.text}]}>Depth</Text>
          ...
          render={({field: {onChange, value}, fieldState: {error}}) => (
            <>
              <TextInput
                ...
                right={
                  <TextInput.Affix
                    text={<Text style={{color: colors.text}}>cm</Text>}
                  />
                }
                style={[
                  styles.textInput,
