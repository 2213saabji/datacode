...
import {
  fetchAgriAppointmentFarmerView,
  fetchAgriAppointmentSellerView,
  deleteFarmerAppointment,
} from '../../../redux/slices/CRM/FarmerCareerRoad/FarmerCareerRoadSlice';

const FarmerMyAppointmentList = ({navigation}) => {
  ...
  const fetchAppointment = async () => {
    try {
      const result = await dispatch(fetchAgriAppointmentFarmerView());
      if (fetchAgriAppointmentFarmerView.fulfilled.match(result)) {
        if (result.payload) {
  ...
  const deleteJob = useCallback(
    async appointmentId => {
      try {
        const result = await dispatch(deleteFarmerAppointment(appointmentId));
        if (deleteFarmerAppointment.fulfilled.match(result)) {
          if (result.payload) {
    ...
    [dispatch, showAlert, deleteFarmerAppointment],
  );
...
export default FarmerMyAppointmentList;
